...
public class CBLNumericImpl extends FieldMapping {
  ...
  protected void marshal(final MarshalContext ctx, final long v, final Endian endian, final boolean isSigned)
      throws MappingException {
    switch (getSize(ctx)){
      case 1 :
        if ((isSigned && (v > Byte.MAX_VALUE || v < Byte.MIN_VALUE)) || (!isSigned && (v > 0xff || v < 0)))
          throwValueOutOfRange(ctx);
        ctx.put(isSigned ? (byte) v : (byte) (v & 0xff));
        break;
      case 2 :
        if ((isSigned && (v > Short.MAX_VALUE || v < Short.MIN_VALUE)) || (!isSigned && (v > 0xffff || v < 0)))
          throwValueOutOfRange(ctx);
        storeValue(ctx, endian, isSigned ? (short) v : (short) (v & 0xffff));
        break;
      case 4 :
        if ((isSigned && (v > Integer.MAX_VALUE || v < Integer.MIN_VALUE)) || (!isSigned && (v > 0xffffffffl || v < 0)))
          throwValueOutOfRange(ctx);
  ...
  private void throwValueOutOfRange(final MarshalContext ctx) throws MappingException {
    throw new MappingException(this, "Numeric value out of range for field length of " + getSize(ctx));
  }

  private void storeValue(final MarshalContext ctx, final Endian endian, final long valueRespectingSign)
      throws MappingException {
    int s = getSize(ctx);
  ...
  protected Object unmarshal(final UnmarshalContext ctx, final byte[] b, final Endian endian, final boolean isSigned)
      throws MappingException {
    switch (getSize(ctx)){
      case 1 :
        long value = b[0];
  ...
  private long extractValue(final byte[] byteArray, final Endian endian, final MappingContext ctx) {
    long result = 0;
    int shift = 0;
    int s = getSize(ctx);
    int offset = endian == Endian.BIG ? s : 0;
